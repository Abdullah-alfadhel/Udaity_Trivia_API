...
#paginating the questions function that take the questions and the request to paginate
...
def create_app(test_config=None):
  ...
  def categories_in_game():
    #fetches the categories and sort them in a dictionary format to be accepted by the font end.
    try:
  ...
  def get_questions():
    #fetches the question and categories form the database to be formated for the font end.
  ...
  def question_delete(question_id):
    #deletes the question by geting the question id and then perform a delete command on it.
    try:
  ...
  def adding_question():
    ...
    if len(request.get_json()['question']) == 0 or len(request.get_json()['answer']) == 0 or request.get_json()['difficulty'] == None or request.get_json()['category'] == None:
      abort(422)
    try:
      #jsonify the request to handle it easily.
      newQuestion = request.get_json()
      question = Question(question=newQuestion['question'],answer=newQuestion['answer'],difficulty=newQuestion['difficulty'],category=newQuestion['category'])
  ...
  def questions_search():
    try:
      ...
      categories = Category.query.all()
      data = {}
      #formating the categories into dictionary to be accepted in the front end
      for category in categories:
        data[category.id] = category.type
  ...
  def play_quizzes():
    try:
      ...
      current_question = questions[random.randint(0,len(questions) -1)].format()
      #cheking term to be used in the cheking loop
      is_it_new = True
      #cheking if the question is in the previous questions and assign a new one.
      while is_it_new:
